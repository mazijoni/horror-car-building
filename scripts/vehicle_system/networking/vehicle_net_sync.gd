...
func rpc_add_part(part_id: String, gp: Vector3i, orient_idx: int, color: Color) -> void:
	if not multiplayer.is_server():
		return
	# Future: rpc("_remote_add_part", part_id, [gp.x, gp.y, gp.z], orient_idx, [color.r, color.g, color.b])
	pass
